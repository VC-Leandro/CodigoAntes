jQuery( document ).ready(function( $ ) {
	...
	function($) {
		...
		$.fn.calendar = function (opt,val) {
			...
			def = $.extend(def, opt);
			var weekDays = 'Domingo Segunda Terça Quarta Quinta Sexta Sábado'.split(' ');
			var yearMonths = 'Janeiro Fevereiro Março Abril Maio Junho Julho Agosto Setembro Outubro Novembro Dezembro'.split(' ');
			var yearMonthsShort = 'Jan Fev Mar Abr Mai Jun Jul Ago Set Out Nov Dez'.split(' ');
			var today = new Date();
			...
			var btn3 = $('<button>')
				...
				.ripple()
				.attr('data-toggle', 'tooltip')
				.attr('title', 'Voltar para o mês atual')
				.click(function(e) {
					e.preventDefault();
					if(isMoving) return;
					var dt = new Date();
			...
			var btn4 = $('<button>')
				...
				.ripple()
				.attr('data-toggle', 'tooltip')
				.attr('title', 'Ver data selecionada')
				.click(function(e) {
					e.preventDefault();
					if(isMoving) return;
